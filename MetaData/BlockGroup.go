...
type BlockGroup struct {
	...
	Generator    uint32      `msg:"generator"`
	PreviousHash []byte       `msg:"preHash"`
	MerkleRoot   []byte       `msg:"merkleRoot"`
	VoteTickets  []VoteTicket `msg:"-"`
	Timestamp    float64      `msg:"timestamp"`
	VoteResult   []uint8        `msg:"VoteResult"`
	BlockHashes  [][]byte     `msg:"BlockHashes"`
	NextDutyWorker	uint32	  `msg:"NextDutyWorker"`
	VoteAggSign []byte      `msg:"voteAggSign"`
	Sig          string       `msg:"Sig"`
	BlockAggSign []byte        `msg:"-"`
	...
	ExecutionResult	  map[string]bool `msg:"-"`
}

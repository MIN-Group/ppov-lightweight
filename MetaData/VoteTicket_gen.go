...
func (z *VoteTicket) UnmarshalMsg(bts []byte) (o []byte, err error) {
	...
	for zb0001 > 0 {
		...
		case "Voter":
			z.Voter, bts, err = msgp.ReadUint32Bytes(bts)
			if err != nil {
				err = msgp.WrapError(err, "Voter")
				return
			}
...
func (z *VoteTicket) Msgsize() (s int) {
	s = 1 + 11 + msgp.ArrayHeaderSize + (len(z.VoteResult) * (msgp.Uint8Size)) + 7 + msgp.ArrayHeaderSize
	for za0002 := range z.BlockHashes {
		s += msgp.BytesPrefixSize + len(z.BlockHashes[za0002])
	}
	s += 6 + msgp.Uint32Size + 10 + msgp.Float64Size + 4 + msgp.StringPrefixSize + len(z.Sig)
	return
}
